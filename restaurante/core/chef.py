...
fogao = threading.Semaphore(1)  # apenas 1 fogão disponível
...
def _preparar_pedido(self, pedido):
    ...
    self.estado = "AGUARDANDO FOGÃO"
    print(f"⏳ [Chef {self.id}] Esperando fogão para preparar pedido {pedido.id}")
    
    with fogao:  # tentativa de adquirir o fogão
        self.estado = "PREPARANDO"
        pedido.estado = EstadoPedido.EM_PREPARO
        print(f"🔥 [Chef {self.id}] Usando fogão para preparar pedido {pedido.id}")
